...
import { flippico } from '../flp-set-new-password/flp-set-new-password.styles';
import { ifDefined } from 'lit/directives/if-defined.js';

/**
 * @summary Set new password v2 — uses base_url attribute for API calls
 * @tag flp-set-new-password-v2
 */
@customElement("flp-set-new-password-v2")
export class FlpSetNewPasswordV2 extends FlpElement {
  ...
  /** Base URL for API calls. Defaults to empty (relative paths). */
  @property({ type: String, attribute: "base_url" }) baseUrl = '';
  ...
  async onSubmitHandle(event: any) {
    ...
    this.errorText = null;
    fetch(`${this.baseUrl}/api/${this.tenantKey}/set-password/${formData.get("token")}`, {
      method: "POST",
      body: urlencoded,
      headers: {
