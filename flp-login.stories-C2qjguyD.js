...
`;var $=Object.defineProperty,S=Object.getOwnPropertyDescriptor,i=(t,r,n,e)=>{for(var a=e>1?void 0:e?S(r,n):r,m=t.length-1,g;m>=0;m--)(g=t[m])&&(a=(e?g(r,n,a):g(a))||a);return e&&a&&$(r,n,a),a};let o=class extends w{constructor(){super(...arguments),this.tenantKey="",this.logo="",this.signUpUrl="",this.resetPasswordUrl="",this.staging=!1,this.develop=!1,this.mobileLogin=!1,this.loginPending=!1}async loginByGoogle(){fetch(`${d(this.staging,this.develop)}/api/${this.tenantKey}/google`,{method:"GET"}).then(t=>{if(t.ok)return t.json();if(t.status===404)throw this.errorText="User not found",new Error("User not found");if(t.status===403)throw this.errorText="Incorrect password",new Error("Incorrect password")}).then(t=>{window.location.href=t.message.redirect_url}).catch(console.error).finally(()=>this.loginPending=!1)}async loginByApple(){console.log("login by apple")}async onSubmitHandle(t){t.preventDefault(),this.loginPending=!0;const r=new FormData(t.target);if(Array.from(r.values()).some(e=>e==="")){this.errorText="Password or email is empty",this.loginPending=!1;return}const n=new URLSearchParams;n.append("tenant_key",this.tenantKey),n.append("email",r.get("email")),n.append("password",r.get("password")),this.errorText=null,fetch(`${d(this.staging,this.develop)}/api/${this.tenantKey}/login`,{method:"POST",body:n,headers:{"Content-Type":"application/x-www-form-urlencoded"}}).then(e=>{if(e.ok)return e.json();if(e.status===404)throw this.errorText="User not found",new Error("User not found");if(e.status===403)throw this.errorText="Incorrect password",new Error("Incorrect password")}).then(e=>{window.parent.postMessage({type:"LOGIN_SUCCESS",payload:e.message},"*"),console.log("SENT POST MESSGAE")}).catch(console.error).finally(()=>this.loginPending=!1)}render(){return p`
...
`,s=T.bind({});s.args=U;var f,u,h;s.parameters={...s.parameters,docs:{...(f=s.parameters)==null?void 0:f.docs,source:{originalSource:`(props: Login, {
  globals: {
    theme
  }
}: {
...
//# sourceMappingURL=flp-login.stories-C2qjguyD.js.map
